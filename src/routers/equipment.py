"""Equipment Router for gym catalog management."""
...
async def create_equipment(in_data: dict, service: EquipmentService = Depends(get_equipment_service)):
...
async def list_equipment(skip: int = 0, limit: int = 100, service: EquipmentService = Depends(get_equipment_service)):
    """List equipment types with pagination."""
    equipment_list, _ = await service.list(skip=skip, limit=min(limit, 100))
...
async def update_equipment(id: UUID, in_data: dict, service: EquipmentService = Depends(get_equipment_service)):
